(function(window)
{
  ...
  class Table
  {
    ...
    setupUI()
    {
      ...
      table.$fixedHeader.html(Table.htmlFixedHeader(table.fixedCols));
      table.$scrollableHeader.html(Table.htmlScrollableHeader(table.scrollableCols));
      ...
      if (table.options.actions && table.options.actions.length > 0) table.$actions.html(Table.htmlTableActions(table.options.actions));
    }
    ...
    setupRowSelectionEventHandlers()
    {
      let table = this;
    ...
    setupFilterPopupEventHandlers()
    {
      ...
      table.$filterPopup.on("keydown", ".search-input input", (evt) =>
      {
        ...
        if (val != "")
        {
          ...
          table.$filterPopup.find(".items").html(Table.htmlFilterLovItems(fuse.search(val).map((i) => i.item)));
          table.$filterPopup.find(".lov-action[zn-lov-action='blanks']").addClass("na");
        }
        else
        {
          table.$filterPopup.find(".items").html(Table.htmlFilterLovItems(ctx.lov));
      ...
      table.$filterPopup.on("click", ".lov .lov-actions .lov-action", (evt) =>
      {
        ...
        if (action == "blanks")
        {
          if (table.$filterPopup.find(".lov .items .checkbox[data-checked='Y']").get().length == ctx.lov.length && checked == "Y")
            $selectAllCheckbox.attr("data-checked", "Y");
      ...
      table.$filterPopup.on("click", ".lov .items .item", (evt) =>
      {
        ...
        if (table.$filterPopup.find(".lov .items .checkbox[data-checked='Y']").get().length == ctx.lov.length && $blanksCheckbox.attr("data-checked") == "Y")
          $selectAllCheckbox.attr("data-checked", "Y");
        else $selectAllCheckbox.attr("data-checked", "N");
      });
    ...
    setRows(rows)
    {
      let table = this;
      table.rows = rows;
      let content = Table.htmlContent(rows, table.fixedCols, table.scrollableCols, table.options.rowActions, table.options.multiSelect);
    ...
    setRowActions(rowActions)
    {
      ...
      {
        if (rowActions.length > 1) rowActionsHTML = Table.htmlRowAction({ action: 'zn-table:show-actions', label: 'Show Actions', icon: 'fas fa-bars' });
        else rowActionsHTML = Table.htmlRowAction(rowActions[0]);
    ...
    showRowActionsMenu($cell, index)
    {
      let table = this;
      let actionMenuItems = Table.htmlActionsMenu(table.options.rowActions);
      ...
      {
        if (!Table.pointInContent({ x: evt.pageX, y: evt.pageY }, table.$rowActionsMenu)) hide();
      });
    ...
    setPages()
    {
      ...
      table.$pageNumbers.html(Table.htmlPages(pageNumbers, p));
    }
    ...
    showFilterPopup($cell, index, column, lov, hasBlanks, filterstate)
    {
      let table = this;
      let popContent = Table.htmlFilterPopup(column.label, lov, hasBlanks, filterstate);
      ...
      {
        if (!Table.pointInContent({ x: evt.pageX, y: evt.pageY }, table.$filterPopup)) table.hideFilterPopup();
      });
    ...
    addFilter(index, column, selectedValues, includeBlanks)
    {
      let table = this;
    ...
    static htmlContent=(rows, fixedcols, scrollablecols, actions, multiselect)=>
    {
      ...
      {
        rowActionsHTML=Table.htmlRowAction(actions.length==1 ? actions[0] : {action: 'zn-table:show-actions', label: 'Show Actions', icon: 'fas fa-bars'});
      }
  
      let multiSelectHTML=null;
      if(multiselect) multiSelectHTML=Table.htmlMultiSelect();
  
      if(rows.length==0) return Table.htmlContentRow(null, 0, fixedcols, scrollablecols, rowActionsHTML, multiSelectHTML);
  
      let content=rows.reduce((a, row, i)=>
      {
        let {fixed, scrollable} = Table.htmlContentRow(row, i, fixedcols, scrollablecols, rowActionsHTML, multiSelectHTML);
        a.fixed += fixed;
  ...
  __package.split(".").reduce((a, e) => a[e] = a[e] || {}, window)[__name] = Table;
